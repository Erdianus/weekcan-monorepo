...
import { sprintFormSchema } from "@hktekno/api/routers/project/sprint/schema";
...
import { optionsTime } from "@hktekno/ui/lib/select";
import useAlertStore from "@hktekno/ui/lib/store/useAlertStore";

const sprintForm = sprintFormSchema
  ...
  const create = k.project.sprint.create.useMutation({
    onSuccess: async ({ message }) => {
      await client.invalidateQueries({
        queryKey: k.project.sprint.all.getKey(),
      });
...
const UpdateSprint = ({
  ...
  const { data: sprint } = k.project.sprint.single.useQuery({
    variables: { id },
  });
  const update = k.project.sprint.update.useMutation({
    onSuccess: async ({ message }) => {
      ...
      await client.invalidateQueries({
        queryKey: k.project.sprint.single.getKey({ id }),
      });
      onClose();
      toast.success(message);
    },
...
const ListSprint = ({ project_id }: { project_id: string }) => {
  ...
  const { data: sprints } = k.project.sprint.all.useQuery({
    ...
    return sprints.data.map((sprint) => {
      ...
      if (dayjs(sprint.start_date).isSame(dayjs(sprint.end_date), "second"))
        return {
          ...
          title: sprint.title,
          allDay,
          date: dayjs(sprint.start_date).format(
            `YYYY-MM-DD${allDay ? "" : "THH:mm:ss"}`,
          ),
        };
      ...
      return {
        ...
        title: sprint.title,
        allDay,
        start: dayjs(sprint.start_date).format(
          `YYYY-MM-DD${allDay ? "" : "THH:mm:ss"}`,
        ),
        end: dayjs(sprint.end_date).format(
          `YYYY-MM-DD${allDay ? "" : "THH:mm:ss"}`,
        ),
      };
    });
      ...
      >
        {formData.id === 0 && (
          <CreateSprint
            date={formData.date}
            onClose={() =>
              setFormData(() => ({ ...formDataInit, open: false }))
            }
          />
        )}
        {formData.id !== 0 && (
