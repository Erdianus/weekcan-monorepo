...
export default function Page() {
  ...
  const param: string | number = useLoaderData<typeof loader>();
  ...
  const update = k.archive.talent.update.useMutation({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    values: {
      name: isLoading ? '' : response!.data.name,
      about: isLoading ? '' : response!.data.about,
      category_id: isLoading ? '' : response!.data.category_id,
      
      contact:  isLoading ? [] : response!.data.contact.map(({id, type, contacts}) => ({key: id.toString(),type, contact: contacts}) ),

      skill: isLoading ? [] : response!.data.skill.map((skill_el) => skill_el.id.toString() ),
      experience: isLoading ? [] : response!.data.experience.map(({id, title, from, to, detail}) => ({key: id.toString(),title, from, to, detail}) ),
      education: isLoading ? [] : response!.data.education.map(({id, title, from, to, detail}) => ({key: id.toString(),title, from, to, detail}) ),
    },
  });
  ...
  return (
    <>
      <h2 className="mb-4 text-3xl font-bold">
        Edit Data {JSON.stringify(form.formState.errors)}
      </h2>

      {isLoading ?

        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%' }}><Spinner /></div> :
        <Form {...form}>
          <form
            className="space-y-6"
